...
- 완료된 것:
  ...
  - R83 카테고리 개편: 5그룹 15항목 단일 소스화, 온보딩 직접 입력, 프로덕션 admin 폴백 차단
- 로컬 미추적 파일:
...
- [x] `/feed` 구현
  ...
  - [x] 프로필 사진 없음
  - [x] 카드별 마감 상태와 핵심 판단 정보 표시
  - [x] 카드 만들기, 내 활동, 알림으로 이어지는 빠른 행동 패널
...
- [x] 하단 네비게이션 공통 컴포넌트화
- [x] 다크 디자인 토큰 1차 적용
- [x] Round 89: 타겟에 맞춰 전체 앱 배경을 화사한 크림-옐로우 계열 그라디언트로 실험 적용
- [x] Round 90: 보라 톤을 걷어내고 화이트 비중이 높은 크림톤 토큰/피드/버튼 색상으로 조정
- [x] Round 91: 하단 메뉴를 불투명 회백 그라디언트로 바꾸고 피드 카드/CTA/마감 색상 대비 조정
- [x] Round 92: CTA를 묵직한 연두 톤으로 조정하고 카드 핵심 문구 박스/Mates 강조 방식 정리
- [x] Round 93: 온보딩 프로필 선택 화면을 밝은 카드 UI로 복구하고 화이트/선명 연두 톤 강화
- [x] Round 94: 카드 투명도 차등을 제거하고 박스 화이트 통일, Mate/Mates 테마색 적용
- [x] Round 95: 화이트 기반에 피치·코랄·약한 라벤더 오로라 배경 실험 적용
- [x] Round 96: Vercel 빌드 안정화를 위해 Google Fonts 빌드 의존성 제거
- [x] Round 97: 키 컬러를 핑크-퍼플 계열로 전환하고 카드 라벤더 테두리/회색 정보 박스 적용
- [x] Round 98: 카드 투명도와 크림 정보 박스 조정, 펼침 색상 플래시 제거
- [x] 로그인/온보딩 1차 리디자인
- [x] Round 99: MVP Scope 문서 신설 (`Mate_MVP_Scope.md`), 휴대폰 인증 MVP 제외 확정(코드는 휴면 유지), PostHog 통계 연동 (핵심 퍼널 이벤트 8종, 키 없으면 no-op)
- [x] Round 107: 피드 카드 본체 투명도 0.80 조정, 핵심 오퍼 박스는 회색 톤 복원, 상세 정보 박스는 크림톤 유지
- [x] Round 108: 피드 상세 정보 박스를 핑크 화이트 톤으로 조정하고 메타 정보 줄 높이를 낮춰 정돈
- [x] Round 109: 피드 상세 정보 박스의 핑크톤을 낮추고 회색조 박스 톤으로 정리
- [x] Round 110: 공통 paper 토큰과 bg-paper 계열 박스를 노란 크림톤에서 회색조로 전환
- [x] Round 111: Mate 만들기 진입/24시간 날짜 선택/공통 하단 네비/Room 메시지 실시간 보조 갱신과 알림 타입 연결
