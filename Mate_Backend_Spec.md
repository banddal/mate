...
>
> **Round 8**: 평점 지수·미팅 성사 지수 산출식 확정(§5) + 현금성 패턴 정규식 확정(§7), `supabase/migrations/20260706000004_rating_functions.sql` 추가. 이 두 함수는 임의 유저의 이력을 반환하므로 `authenticated`에는 EXECUTE 권한을 주지 않고 service_role 전용으로 잠갔다(그렇지 않으면 "경쟁 시에만 노출" 규칙이 직접 RPC 호출로 우회됨).
...
- **평점 지수**·**미팅 성사 지수** (Round 8 확정, `supabase/migrations/20260706000004_rating_functions.sql`)
  - 미팅 성사 지수 = `승인된 신청 중 실제 참석 비율(%)` — "이 사람을 뽑으면 실제로 올 확률"
  - 평점 지수 = `참석한 모임 중 시간을 지킨 비율(%)` — `matches_description`/`would_rejoin`은 이 사람이 아니라 호스트/카드에 대한 평가라 의도적으로 제외(P5 매력·호감 점수 금지 원칙)
  - 둘 다 표본 2건 미만이면 `null`("신규" 배지로 표시 — 표본 1건짜리 극단값 노출 방지)
  - 이 두 함수는 임의 유저의 이력을 반환하므로 `authenticated`에 EXECUTE 권한을 주지 않는다 — **service_role(Route Handler)만 호출 가능**, 그래야 "경쟁 시에만 노출" 규칙이 우회되지 않음
...
### 현금성 패턴 정규식 (Round 8 확정 — 키워드 회피 대응)
`banned_words`의 "사례비/페이/용돈/보상" 같은 정확한 단어 매칭은 "5만원 드립니다"처럼 키워드 없이 금액+지급 의사만 표현한 문장을 놓친다. 아래 두 정규식이 **title/description/host_offer/cost_info 전체**에 대해 **둘 다** 매칭되면 `flag`로 처리(§7 흐름의 2단계):

```
금액 표현:     /\d+\s*(원|만원|천원)/
지급 의사 표현: /(드립니다|드려요|드릴게요|지불|송금|입금|사례|페이|용돈|보상)/
```

운영 데이터가 쌓이면 오탐/누락 사례로 목록을 계속 조정한다(§13 다음 라운드 후보에 유지).
...
## 12. 다음 라운드 후보 (백엔드)
- **법정 최소 로그 보존 기간 — 법무 검토 필요.** 제가 임의로 숫자를 정할 사안이 아님(통신비밀보호법·정보통신망법 등 관련 법령 검토가 선행되어야 함). `retention_archive`는 이미 만들어져 있으니, 기간만 정해지면 삭제 배치 정책에 반영
- 현금성 패턴 정규식은 초안 확정(§7)했지만 운영 데이터가 쌓이면 오탐/누락 기준으로 계속 조정 필요
- L3 오픈을 실제로 결정하는 시점 — 부록 A 원안 재검토
- ~~평점 지수·미팅 성사 지수 산출식~~ — Round 8에서 확정 (§5, `20260706000004_rating_functions.sql`)
